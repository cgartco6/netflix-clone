...
// Curated Expert Highlights for Hero Carousel (Real verified links/channels)
const HERO_FEATURED = [
  {
    id: 'f1',
...
export default function Home() {
  ...
  const [modalMode, setModalMode] = useState('watch'); // 'watch', 'listen', 'lyrics'
  const [searchQuery, setSearchQuery] = useState('');
  const [localDriveItems, setLocalDriveItems] = useState([]);

  // Auto-scroll hero every 6 seconds
  useEffect(() => {
  ...
  return (
    <div style={styles.container}>
      <Head>
        <title>Netflix Universal Media</title>
      </Head>
      ...
      <header style={styles.header}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '30px' }}>
          ...
          <nav style={styles.navLinks}>
            {['all', 'movies', 'aitech', 'antarctica', 'music'].map((t) => (
              <span
                key={t}
                style={activeTab === t ? styles.activeNav : styles.navLink}
                onClick={() => setActiveTab(t)}
              >
                {t === 'all' && 'Home'}
                {t === 'movies' && 'Movies & Series'}
                {t === 'aitech' && 'Expert AI & Tech'}
                {t === 'antarctica' && 'Antarctica Science'}
                {t === 'music' && 'Radio & Lyrics'}
              </span>
            ))}
          </nav>
        </div>
      ...
      {/* Dynamic Billboard Hero Section */}
      <div style={{ ...styles.billboard, backgroundImage: `linear-gradient(to top, #141414, transparent 60%), url("${hero.bgImage}")` }}>
          ...
          <div style={styles.modalContent} onClick={(e) => e.stopPropagation()}>
            ...
            <div style={styles.modalHeader}>
              ...
              <div>
                ...
                <p style={styles.genreTag}>{selectedItem.genre}</p>
                <p>{selectedItem.description}</p>
                {selectedItem.cast && <p><strong>Cast:</strong> {selectedItem.cast.join(', ')}</p>}
                {selectedItem.author && <p><strong>Verified Source:</strong> {selectedItem.author}</p>}
              </div>
            </div>
...
const styles = {
  ...
  header: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '15px 50px', backgroundColor: 'rgba(0,0,0,0.9)', position: 'sticky', top: 0, zIndex: 100 },
  logo: { color: '#E50914', margin: 0, fontSize: '30px', fontWeight: 'bold', letterSpacing: '1px' },
  navLinks: { display: 'flex', gap: '20px' },
  navLink: { color: '#E5E5E5', cursor: 'pointer', fontSize: '14px' },
  activeNav: { color: '#FFF', fontWeight: 'bold', cursor: 'pointer', fontSize: '14px', borderBottom: '2px solid #E50914' },
  ...
  searchBox: { display: 'flex', backgroundColor: '#000', borderRadius: '4px', overflow: 'hidden' },
  ...
  billboard: { height: '50vh', backgroundSize: 'cover', backgroundPosition: 'center', display: 'flex', alignItems: 'flex-end', padding: '0 50px 40px 50px', transition: 'background-image 0.8s ease-in-out' },
  billboardContent: { maxWidth: '600px' },
  badge: { color: '#E50914', fontWeight: 'bold', fontSize: '12px', letterSpacing: '1px' },
  billboardTitle: { fontSize: '36px', margin: '10px 0', fontWeight: 'bold' },
  billboardDesc: { fontSize: '14px', color: '#DDD', lineHeight: '1.4' },
  playBtn: { backgroundColor: '#FFF', color: '#000', border: 'none', padding: '10px 24px', fontSize: '16px', fontWeight: 'bold', borderRadius: '4px', cursor: 'pointer', marginTop: '10px' },
  main: { padding: '20px 50px' },
  row: { marginBottom: '35px' },
  rowTitle: { fontSize: '20px', fontWeight: 'bold', marginBottom: '15px' },
  slider: { display: 'flex', gap: '15px', overflowX: 'auto', paddingBottom: '10px' },
  card: { flex: '0 0 180px', cursor: 'pointer' },
  cardImgWrapper: { width: '150px', height: '220px', borderRadius: '4px', overflow: 'hidden', backgroundColor: '#222' },
  cardImg: { width: '100%', height: '240px', objectFit: 'cover', borderRadius: '4px' },
  cardText: { fontSize: '13px', fontWeight: 'bold', margin: '6px 0 0 0', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' },
  cardSub: { fontSize: '11px', color: '#888', margin: 0 },
  top10Card: { display: 'flex', alignItems: 'center', cursor: 'pointer', flex: '0 0 220px' },
  rankNum: { fontSize: '100px', fontWeight: '900', color: '#141414', WebkitTextStroke: '3px #555', lineHeight: '0.8', marginRight: '-20px', zIndex: 1 },
  ...
  genreTag: { display: 'inline-block', backgroundColor: '#E50914', padding: '3px 8px', borderRadius: '3px', fontSize: '11px', fontWeight: 'bold' },
  modalTabs: { display: 'flex', gap: '10px', marginBottom: '15px', borderBottom: '1px solid #333', paddingBottom: '10px' },
  tabBtn: { backgroundColor: '#222', color: '#AAA', border: 'none', padding: '8px 16px', borderRadius: '4px', cursor: 'pointer' },
  activeTabBtn: { backgroundColor: '#E50914', color: '#FFF', border: 'none', padding: '8px 16px', borderRadius: '4px', cursor: 'pointer', fontWeight: 'bold' },
  modalBody: { backgroundColor: '#111', padding: '15px', borderRadius: '4px' },
  lyrics: { whiteSpace: 'pre-wrap', color: '#00FF66', fontFamily: 'monospace', fontSize: '13px' }
};
